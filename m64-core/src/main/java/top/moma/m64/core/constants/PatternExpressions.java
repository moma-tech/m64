 ...
 * PatternExpressions
 *
 * <p>Pattern Expressions
 *
 * <p>https://any86.github.io/any-rule/
 *
 * @author ivan
 * @version 1.0 Created by ivan at 11/23/20.
 */
public interface PatternExpressions {
  /** 英文字母 、数字和下划线 */
  ...
  /** 中文汉字 */
  Pattern CHINESES = Pattern.compile("[\u4E00-\u9FFF]+");
  /** 分组 */
  ...
  /** 邮编 */
  Pattern ZIP_CODE = Pattern.compile("[1-9]\\d{5}(?!\\d)");
  /** 生日 */
  Pattern BIRTHDAY = Pattern.compile("^(\\d{2,4})([/\\-.年]?)(\\d{1,2})([/\\-.月]?)(\\d{1,2})日?$");
  /** URL */
  Pattern URL = Pattern.compile("[a-zA-z]+://[^\\s]*");
  /** Http URL */
  Pattern URL_HTTP =
  ...
  Pattern UUID =
      Pattern.compile(
          ...
          Pattern.CASE_INSENSITIVE);
  /** 不带横线的UUID */
  Pattern UUID_SIMPLE = Pattern.compile("^[0-9a-f]{32}$", Pattern.CASE_INSENSITIVE);
  /** MAC地址正则 */
  Pattern MAC_ADDRESS =
      Pattern.compile(
          ...
          Pattern.CASE_INSENSITIVE);
  /** 16进制字符串 */
  Pattern HEX = Pattern.compile("^[a-f0-9]+$", Pattern.CASE_INSENSITIVE);
  /** 时间正则 */
   ...
   * </pre>
   */
  Pattern CREDIT_CODE = Pattern.compile("^[0-9A-HJ-NPQRTUWXY]{2}\\d{6}[0-9A-HJ-NPQRTUWXY]{10}$");
}
